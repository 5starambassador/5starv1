...
import { Home, UserPlus, BarChart3, List, User } from 'lucide-react'
...
export function BottomNav() {
    ...
    const isActive = (path: string) => {
        if (path === '/' || path === '/dashboard' || path === '/campus' || path === '/superadmin' || path === '/admin') {
    ...
    // Determine Dashboard Link based on simple logic (Actual role logic is handled by redirect in layout/page, 
    // but for nav highlighting we can check current path or default to dashboard)
    // A smarter way is to just have a "Home" link that goes to root '/' and let middleware/page redirect.
