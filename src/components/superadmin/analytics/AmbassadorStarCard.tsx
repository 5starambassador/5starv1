        ...
        <div className="bg-white rounded-[32px] border border-gray-100 shadow-[0_10px_15px_-3px_rgba(0,0,0,0.05)] p-8 h-full flex flex-col">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
                ...
                <div className="flex flex-col">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4 px-1">Global Tier Mix</p>
                            ...
                            milestones.risingStars.slice(0, 5).map((star: any) => (
                                <div key={star.name} className="bg-gray-50/50 border border-gray-100/50 rounded-2xl p-4 flex items-center justify-between group hover:border-gray-200 hover:bg-white transition-all shadow-sm hover:shadow-md cursor-default">
                                    <div className="flex items-center gap-4">
                                        <div className="w-11 h-11 rounded-full bg-white flex items-center justify-center font-black text-sm shadow-sm border border-gray-100 text-gray-700 uppercase">
                                            {star.name.charAt(0)}
                                        </div>
                                        <div>
                                            <p className="font-black text-sm leading-tight text-gray-900 tracking-tight">{star.name}</p>
                                            <p className="text-[10px] text-gray-400 mt-1 uppercase tracking-wider font-black">{star.campus}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-[11px] font-bold text-gray-500 mt-1">to reach <span className="text-gray-900 font-black">{star.nextTier}</span></p>
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="h-full flex flex-col items-center justify-center py-8 opacity-40 bg-gray-50/50 rounded-3xl border border-dashed border-gray-200">
