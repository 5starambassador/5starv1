                    ...
                    {preAsset.length === 0 ? (
                        ...
                        <div className="space-y-6">
                            {preAsset.map((referral: any) => (
                                <ReferralCard key={referral.leadId} referral={referral} type="pre-asset" user={user} />
                            ))}
                        </div>
                    )}
                </PageAnimate>
                    ...
                    {asset.length === 0 ? (
                        ...
                        <div className="space-y-6">
                            {asset.map((referral: any) => (
                                <ReferralCard key={referral.leadId} referral={referral} type="asset" user={user} />
                            ))}
                        </div>
                    )}
                </PageAnimate>
...
function ReferralCard({ referral, type, user }: { referral: any, type: 'pre-asset' | 'asset', user: any }) {
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <a 
                            href={`https://wa.me/${referral.parentMobile}?text=${encodeURIComponent(`Hello! I'm *${user?.fullName || 'an Ambassador'}*, an Ambassador from ACHARIYA. I'm reaching out regarding your referral for *${referral.studentName || 'your child'}*. I'd love to share an update on the admission status!`)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-10 h-10 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 hover:bg-emerald-500/20 hover:scale-110 active:scale-95 transition-all shadow-lg"
